...
from openpype.pipeline import legacy_io, Anatomy, schema
...
def build_assets_library(project_name: str):
    ...
    Crawls all blend reprensentations of current site and see if marked as asset
    to create a symlink of blend file to the Assets Library directory.
    ...
    marked_subsets = [
        s
        for s in get_subsets(project_name, fields=["data.blender"])
        if s["data"].get("blender", {}).get("marked_as_asset")
    ]
    for subset in marked_subsets:
        last_version = get_last_version_by_subset_id(
            project_name, subset["_id"]
        )
        ...
        version_file = Path(
            blend_representation.get("files")[0]["path"].format_map(
                {"root": anatomy.roots}
            )
        )
